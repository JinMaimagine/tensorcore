...
\begin{itemize}
    ...
    \item 王俊涵: 5\%
\end{itemize}
后期:马晋准备单独添加对于bf16和int16的支持(全部由马晋完成)


新增支持:
1.现在支持BF16(可以混合精度和非混合精度),INT16(在main.cpp中必须设置mixed=1)
2.改动了PE的逻辑:新增simpleout模块,这个模块的作用是,在输入稀疏的时候,可以做到低功耗,因为保持了MAC单元的输入不会bit翻转,而是直接从simpleout得到结果
当然,由于新增了逻辑,PE中的a,b也由原来的组合逻辑变成了时序逻辑,因此对PE的逻辑也进行了修改。
\end{document}